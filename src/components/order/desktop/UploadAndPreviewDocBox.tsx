...
import { useOrderPrintStore, useOrderWorkflowStore } from '@states/order';
import { formatFileSize } from '@utils';

export function UploadAndPreviewDocBox() {
  ...
  const { openDesktopOrderWorkflow, setOpenDesktopOrderWorkflow } = useOrderWorkflowStore();
  ...
  return (
    <Dialog
      size='xl'
      open={openDesktopOrderWorkflow}
      handler={() => setOpenDesktopOrderWorkflow(!openDesktopOrderWorkflow)}
    >
      <DialogHeader className='justify-end p-0 bg-gray-200 rounded-t-md'>
        <IconButton variant='text' onClick={() => setOpenDesktopOrderWorkflow(false)}>
